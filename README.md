...
A web component for visualizing **C and C++ code execution** with an enhanced [Python Tutor](https://pythontutor.com/) engine.  
...
- **C / C++ Support** — Backend scripts generate execution traces for C and C++ programs.
...
If you want to generate C++ traces for a single page, use `viztrace_cpp.py` instead. It works the same way but writes `code.cpp` and requests C++ traces from the backend.

**(b) Structured chapters (batch mode) for C** <br>
...
If you want the same batch flow for C++, use `viztrace_cpp_folder.py`.
...
```
- Set `lang="cpp"` when rendering C++ traces generated by `viztrace_cpp.py` / `viztrace_cpp_folder.py`.
...
```plaintext
pythontutor-c-webcomponent/
├── build/                     # Compiled JS/CSS assets and visualizer runtime
├── example/                   # Generated traces for single-page demos
│   └── test-component/example1/
│       ├── code.c
│       └── trace.json
├── docs/                      # Demo screenshots
├── loader.js                  # Single-page CDN loader
├── loader_folder.js           # Multi-page / textbook CDN loader
├── viztrace_c.py              # Generate C traces from one HTML page
├── viztrace_c_folder.py       # Generate C traces from a folder of pages
├── viztrace_cpp.py            # Generate C++ traces from one HTML page
├── viztrace_cpp_folder.py     # Generate C++ traces from a folder of pages
...
Notes:
- `build/visualizerComponent.js` is the single-page custom element implementation.
- `build/visualizerComponent_folder.js` is the textbook / folder-mode implementation.
- The `build/` directory is checked-in build output, not the original source tree.
---
## 🐞 Contact
